defmodule GameBox do
  ...
  def start_arena(_host_player_id) do
    code = generate_code()
    {:ok, _pid} = GameBox.Arenas.Server.start_link(code)
    {:ok, _plugin} = load_game(code, "games/tictactoe/target/wasm32-unknown-unknown/debug/tictactoe_rs.wasm")
  ...
  def load_game(code, path) do
    GameBox.Arenas.Server.load(code, %{ wasm: [ %{ path: path } ]}, false)
  end
  ...
  def init_game(code, player_ids) do
    GameBox.Arenas.Server.plugin_apply(code, {:call, "init_game", Jason.encode!(%{player_ids: player_ids})})
  end
  ...
  def render_game(code, assigns) do
    GameBox.Arenas.Server.plugin_apply(code, {:call, "render", Jason.encode!(assigns)})
  end
  ...
  def handle_game_event(code, event) do
    ...
    result = GameBox.Arenas.Server.plugin_apply(code, {:call, "handle_event", event})
    Jason.decode!(result, keys: :atoms)
  end

  def fetch(code) do
    GameBox.Arenas.Registry.whereis_name(code)
  end

  defp generate_code do
    code = for _ <- 1..4, into: "", do: <<Enum.random('ABCDEFGHIJKLMNOPQRSTUVWXYZ')>>

    if GameBox.Arenas.Server.exists?(code) do
      generate_code()
    else
      code
    end
