defmodule GameBoxWeb.CoreComponents do
  @moduledoc """
  ...
  def flash(assigns) do
    ...
    >
      <p :if={@title} class="flex items-center gap-1.5 text-[0.8125rem] font-semibold leading-6">
        <Heroicons.information_circle :if={@kind == :info} mini class="h-4 w-4" />HEY GIRL
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(GameBoxWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(GameBoxWeb.Gettext, "errors", msg, opts)
    end
  end
